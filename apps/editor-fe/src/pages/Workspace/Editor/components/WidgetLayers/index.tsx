...
const WidgetLayers: FC = () => {
	...
	function changeTitle(event: ChangeEvent<HTMLInputElement>) {
		...
		dispatch(changeWidgetTitle({ fe_id: selectedId, title: newTitle }));
	}

	// 切换 隐藏/显示
	function changeHidden(fe_id: string, isHidden: boolean) {
		dispatch(changeWidgetHidden({ fe_id, isHidden }));
	}

	// 切换 锁定/解锁
	function changeLocked(fe_id: string) {
		dispatch(toggleWidgetLocked({ fe_id }));
	}

	// SortableContainer 组件的 items 属性，需要每个 item 都有 id
	const componentListWithId = widgetsList.map((c) => {
		return { ...c, id: c.fe_id };
	});
